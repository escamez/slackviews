"""
Class with nosetests for SelectMenu AbstractBlock in slack_view library
"""
from nose.tools import raises

from slackviews.view import PlainText, Option, SelectMenu, Confirmation
...
class TestSelectMenu:
    ...
    def setup(self):
        ...
        _builder = SelectMenu.Builder().action_id(self.expected_action_id).placeholder(self.expected_placeholder)
        self.selectmenu_instance_required = _builder.build()
        ...
        self.selectmenu_instance_all = _builder.build()

        self.expected_serialized_dict = {'type': 'static_select',
        ...
        self.expected_serialized_json = f'{{"type": "static_select", "confirm": {{"deny": {{"type": "plain_text", ' \
    ...
    def teardown(self):
        SelectMenu.__all_slots__ = None

    def test_should_selectmenu_builder_provide_a_valid_instance_with_required_values(self):

        # GIVEN
        expected_slots = ['_placeholder', '_action_id']
        expected_all_slots = ['_options', '_option_groups', '_initial_option', '_confirm']
        expected_all_slots.extend(expected_slots)

        # WHEN
        instance = self.selectmenu_instance_required

        # THEN
        assert isinstance(instance, SelectMenu)
    ...
    def test_should_selectmenu_serialize_raise_assert_error_when_both_mutually_exclusive_options_are_given(self):
        # GIVEN
        selectmenu = self.selectmenu_instance_required
        setattr(selectmenu, '_options', 'any options')
        setattr(selectmenu, '_option_groups', 'an option group when options exists')

        # WHEN
        selectmenu.serialize()

    def test_should_selectmenu_builder_provide_a_valid_instance_with_any_values(self):

        # GIVEN
        instance = self.selectmenu_instance_all
    ...
    def test_should_selectmenu_serialize_raise_attributeerror_if_trying_to_serialize_without_all_required_slots(self):

        # WHEN
        SelectMenu.Builder().build().serialize()

    def test_should_selectmenu_serialize_provide_correct_dict_and_json_data(self):
    ...
    def test_should_selectmenu_deserialize_provide_correct_instances_from_dict_and_json(self):
        ...
        # WHEN
        instance_from_dict = SelectMenu.deserialize(serialized_dict)
        instance_from_json = SelectMenu.deserialize(serialized_json, from_json=True)
    ...
    def test_should_builder_create_instance_if_option_groups_are_used_instead_of_options(self):
        ...
        # WHEN
        instance = SelectMenu.Builder().placeholder('any placeholder').action_id('any actionid') \
    ...
    def test_should_serialize_instance_work_correctly_if_option_groups_are_used_instead_of_options(self):
        ...
        # GIVEN
        expected_serialized_dict = {'type': 'static_select',
        ...
        expected_serialized_json = '{"type": "static_select", "option_groups": [{"options": [{"value": "any value", ' \
    ...
    def test_should_get_default_provide_correct_initial_option_if_optiongroups_defined(self):
        ...
        default_by_text = SelectMenu.Builder().action_id('any actionid').placeholder('any placeholder') \
            .OptionGroup__().label('any label').Option().text(expected_text).value(expected_value).up() \
            .up().initial_option_(expected_text).build()
        default_by_value = SelectMenu.Builder().action_id('any actionid').placeholder('any placeholder') \
            .OptionGroup__().label('any label').Option().text(expected_text).value(expected_value).up() \
    ...
    def test_should_has_option_provide_correct_answer_if_initial_option_is_defined(self):
        ...
        # GIVEN
        instance_without_options = self.selectmenu_instance_required
        instance_with_options = self.selectmenu_instance_all
    ...
    def test_should_initial_option_raise_attributeerror_if_no_option_or_optiongroups_exists(self):
        ...
        # GIVEN
        _builder =  SelectMenu.Builder().placeholder('any placeholder').action_id('any action_id')
